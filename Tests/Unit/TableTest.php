...
class TableTest extends \PHPUnit\Framework\TestCase
	...
	public function testAll() : void
		...
		$input = [];
		$sql = $showSequenceTable->getSql($input);
		...
		$this->assertEquals('SELECT `showSequence`.*,`artist`.`artistId` as `artist_artistId`,`artist`.`artist`,`artist`.`plays`,`artist`.`rank` FROM `showSequence` LEFT JOIN `artist` ON `artist`.`artistId` = `showSequence`.`artistId`', $sql, 'SQL invalid for addJoin("artist")');
		...
		$this->assertEquals('SELECT `showSequence`.*,`artist`.`artistId` as `artist_artistId`,`artist`.`artist`,`artist`.`plays`,`artist`.`rank`,`title`.`titleId` as `title_titleId`,`title`.`plays` as `title_plays`,`title`.`rank` as `title_rank`,`title`.`title` FROM `showSequence` LEFT JOIN `artist` ON `artist`.`artistId` = `showSequence`.`artistId` LEFT JOIN `title` ON `title`.`titleId` = `showSequence`.`titleId`', $sql, 'SQL invalid for addJoin("artist")');
		...
		$this->assertEquals('SELECT `showSequence`.*,`artist`.`artistId` as `artist_artistId`,`artist`.`artist`,`artist`.`plays`,`artist`.`rank`,`title`.`titleId` as `title_titleId`,`title`.`plays` as `title_plays`,`title`.`rank` as `title_rank`,`title`.`title` FROM `showSequence` LEFT JOIN `artist` ON `artist`.`artistId` = `showSequence`.`artistId` LEFT JOIN `title` ON `title`.`titleId` = `showSequence`.`titleId` LIMIT 10', $sql, 'SQL invalid for setLimit(10)');
		...
		$this->assertEquals('SELECT `showSequence`.*,`artist`.`artistId` as `artist_artistId`,`artist`.`artist`,`artist`.`plays`,`artist`.`rank`,`title`.`titleId` as `title_titleId`,`title`.`plays` as `title_plays`,`title`.`rank` as `title_rank`,`title`.`title` FROM `showSequence` LEFT JOIN `artist` ON `artist`.`artistId` = `showSequence`.`artistId` LEFT JOIN `title` ON `title`.`titleId` = `showSequence`.`titleId` LIMIT 1000, 10', $sql, 'SQL invalid for setOffset(1000)');
		...
		$this->assertEquals('SELECT `showSequence`.*,`artist`.`artistId` as `artist_artistId`,`artist`.`artist`,`artist`.`plays`,`artist`.`rank`,`title`.`titleId` as `title_titleId`,`title`.`plays` as `title_plays`,`title`.`rank` as `title_rank`,`title`.`title` FROM `showSequence` LEFT JOIN `artist` ON `artist`.`artistId` = `showSequence`.`artistId` LEFT JOIN `title` ON `title`.`titleId` = `showSequence`.`titleId` WHERE `sequence` = ? LIMIT 100, 10', $sql, 'SQL invalid for where sequence=2 with setOffset(100)');
		$this->assertEquals(10, $showSequenceTable->getDataObjectCursor()->count(), 'bad count with offset on showSequence table');
	...
	public function testFind() : void
		...
		$input = [];
		$sql = $artistTable->getSql($input);
		$this->assertEquals(1, $records->count(), 'bad count with find artist Beatles');
		$this->assertEquals(['Beatles'], $input, '$input is not ["Beatles"] ' . __METHOD__);
		$this->assertEquals('SELECT * FROM `artist` WHERE `artist` = ?', $sql);
		...
		$this->assertEquals('SELECT * FROM `artist` WHERE `artist` = ? AND `rank` = ?', $sql);
	...
	public function testGroupBy() : void
		...
		$this->assertEquals('SELECT `showSequence`.*,`artist`.`artistId` as `artist_artistId`,`artist`.`artist`,`artist`.`plays`,`artist`.`rank` FROM `showSequence` LEFT JOIN `artist` ON `artist`.`artistId` = `showSequence`.`artistId` GROUP BY `artist`.`artist`', $sql, 'SQL invalid for addJoin("artist") with group by artist.artist');
		$artistTable = new \App\Table\Artist();
